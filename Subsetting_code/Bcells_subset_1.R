...
#CITE-Seq (1) analysis
#Subsetting 3555 cells based on CD19_RNA > 2
...
####Subset by RNA feature expression of Cd19####
DefaultAssay(experiment.lc.plain) <- "ADT"
B_cells <- subset(experiment.lc.plain, subset = Cd19 > 2)
B_cells
head(B_cells[[]])
...
#Or
experiment = SCTransform(B_cells, verbose = TRUE)
experiment[["SCT"]]
...
B_cells <- FindNeighbors(B_cells, dims = 1:50)
...
B_cells <-RunUMAP(B_cells, dims = 1:50, assay = 'RNA', reduction.name = 'rna.umap', reduction.key = 'rnaUMAP_')
...
B_cells <- ScaleData(B_cells)
B_cells <- RunPCA(B_cells, reduction.name = 'apca')
...
##ADT clustering
B_cells <- FindNeighbors(B_cells, dims = 1:25, reduction = "apca")
B_cells <- FindClusters(B_cells, resolution = 2.0, verbose = FALSE) #2 for the resolution
clustree(B_cells, prefix = "ADT_snn_res.") + theme(legend.position="bottom")
B_cells <- RunUMAP(B_cells, reduction = 'apca', dims = 1:25, assay = 'ADT', reduction.name = 'adt.umap', reduction.key = 'adtUMAP_')
...
##WNN clustering
B_cells <- FindClusters(B_cells, graph.name = "wsnn", algorithm = 3, resolution = 1.5, verbose = TRUE) #1.5 for the resolution
...
B_cells = B_cells %>%
  RunUMAP(reduction = "harmony.rna", dims = 1:50, assay = 'RNA', reduction.name = 'harmony.rna.umap', reduction.key = 'harmony.rnaUMAP_') %>%
  FindNeighbors(reduction = "harmony.rna", dims = 1:50) %>%
...
B_cells = B_cells %>%
  RunUMAP(reduction = "harmony.adt", dims = 1:25, assay = 'ADT', reduction.name = 'harmony.adt.umap', reduction.key = 'harmony.adtUMAP_') %>%
  FindNeighbors(reduction = "harmony.adt", dims = 1:25) %>%
...
B_cells <- FindMultiModalNeighbors(
  ...
  dims.list = list(1:50, 1:25), modality.weight.name = "harmony.weight", weighted.nn.name = "harmony.weighted.nn", snn.graph.name = "harmony.wsnn")
B_cells <- FindClusters(B_cells, graph.name = "harmony.wsnn", algorithm = 3, resolution = 1.5, verbose = TRUE) #1.5 for the resolution
...
B_cells
head(B_cells[[]])
SaveH5Seurat(B_cells, filename = "B_cells", overwrite = TRUE)
ggsave("B_cells_p6.png", width = 20, height = 10)

#Or
B_cells <- LoadH5Seurat("B_cells.h5seurat")
...
Idents(B_cells) <- B_cells$harmony.wsnn_res.1.5
B_cells<- RenameIdents(B_cells, `0` = "Cluster 0", `1` ="Cluster 1", `2` ="Cluster 2", `3` = "Cluster 3", `4` = "Cluster 4",`5` ="Cluster 5", `6` = "Cluster 6", `7` = "Cluster 7", `8` = "Cluster 8", `9` = "Cluster 9", `10` = "Cluster 10", `11` = "Cluster 11", `12` = "Cluster 12", `13` = "Cluster 13")
...
##By batch
Batches <- DimPlot(B_cells, reduction = "harmony.wnn.umap", group.by = "Batch")

##Cell numbers and percent by genotype by clusters - harmony.wnn)
head(B_cells[[]])
cell.numbers <- table(B_cells@meta.data$harmony.wsnn_res.1.5, B_cells@meta.data$Genotype)
cell.numbers <- as.data.frame.matrix(cell.numbers)
row.names(cell.numbers) = c("Cluster 0", "Cluster 1", "Cluster 2", "Cluster 3", "Cluster 4", "Cluster 5", "Cluster 6", "Cluster 7", "Cluster 8", "Cluster 9", "Cluster 10", "Cluster 11", "Cluster 12", "Cluster 13")
...
VlnPlot(B_cells, features = "Cxcr4", pt.size = 1)
...
B_cells_6 + IgM + IgD + CD23 + CD21 + CD93
B_cells_6 + CD95 + CD38 + CD86 + CXCR4 + AID
B_cells_6 + PD_L2 + CD80 + Nt5e
B_cells_6 + CTLA4 + Ctla4
B_cells_6 + CD80 + CD86 + PD_L1 + CXCR5 + CD44
...
VlnPlot(B_cells, features = "Igkv5-39") + ggtitle("Igkv5-39") + NoLegend() + theme(plot.title = element_text(color="black", size=15, face="bold"))
VlnPlot(B_cells, features = "Fcer1g") + ggtitle("Fcer1g") + NoLegend() + theme(plot.title = element_text(color="black", size=15, face="bold"))
VlnPlot(B_cells, features = "Ighg3") + ggtitle("Ighg3") + NoLegend() + theme(plot.title = element_text(color="black", size=15, face="bold"))
VlnPlot(B_cells, features = "Cd300lf") + ggtitle("") + NoLegend() + theme(plot.title = element_text(color="black", size=15, face="bold"))
VlnPlot(B_cells, features = "Cd80") + ggtitle("") + NoLegend() + theme(plot.title = element_text(color="black", size=15, face="bold"))
Ctla4 <- VlnPlot(B_cells, features = "Ctla4") + ggtitle("CTLA-4") + NoLegend() + theme(plot.title = element_text(color="black", size=15, face="bold"))
VlnPlot(B_cells, features = "Cyp11a1") + ggtitle("") + NoLegend() + theme(plot.title = element_text(color="black", size=15, face="bold"))
...
head(B_cells[[]])
Idents(object = B_cells) <- B_cells$harmony.wsnn_res.1.5
B_cells$seurat_clusters <- B_cells$harmony.wsnn_res.1.5
...
##Most abundant genes
DefaultAssay(B_cells)<-"RNA"
Bcells_SCT_c3_TFIDF <- WhichCells(object = B_cells, ident = 3)
Bcells_SCT_c3_TFIDF_genes <- tfidf(GetAssayData(B_cells), Bcells_SCT_c3_TFIDF, colnames(B_cells))
...
FeaturePlot(B_cells, features = "Serpinb1a", reduction = "harmony.wnn.umap")
VlnPlot(B_cells, features = "Ccr6", cols = turbo(14)) + NoLegend()
...
CD5 <- FeaturePlot(B_cells, features = "Notch2", reduction = "harmony.wnn.umap", cols = col_con, pt.size = 1.2) + theme_void()
...
FeaturePlot(B_cells, features = "Gsk3b", reduction = "harmony.wnn.umap", cols = col_con, pt.size = 1.2) + theme_void() + theme(plot.title = element_text(color="black", size=15))
...
elephant  <- VlnPlot(B_cells, features = "Cyp11a1", pt.size  = 0.5, group.by = "seurat_clusters") +
...
#Appears to be an enrichment for naive cells in cluster 12
...
B1a_plot1 <- VlnPlot(B_cells, c("B1a1"), pt.size  = 0.5, cols = turbo(14)) + NoLegend() + ggtitle("Enrichment for B1a signature") + theme(plot.title = element_text(color="black", size=15, face="bold"))
...
B1_Bcells_plot1 <- VlnPlot(B_cells, c("B1_Bcells1"), pt.size  = 0.5, cols = turbo(14)) + NoLegend() + ggtitle("Enrichment for B1 signature") + theme(plot.title = element_text(color="black", size=15, face="bold"))
...
Breg_plot1 <- VlnPlot(B_cells, c("Breg1"), pt.size  = 0.5, cols = turbo(14)) + NoLegend() + ggtitle("Enrichment for Breg signature") + theme(plot.title = element_text(color="black", size=15, face="bold"))
...
VlnPlot(B_cells, c("Breg_overlapping1"), pt.size  = 0.5, cols = turbo(14))
...
VlnPlot(B_cells, c("Breg_neg_imm_reg1"), pt.size  = 0.5, cols = turbo(14))
...
VlnPlot(B_cells, c("B101"), pt.size  = 0.5, cols = turbo(14))
...
VlnPlot(B_cells, c("MZ_Bcells1"), pt.size  = 0.5, cols = turbo(14))
...
VlnPlot(B_cells, c("GSE23925_LIGHT_ZONE_VS_NAIVE_BCELL_UP1"), pt.size  = 0.5, cols = turbo(14))
...
VlnPlot(B_cells, c("GSE23925_DARK_ZONE_VS_NAIVE_BCELL_UP1"), pt.size  = 0.5, cols = turbo(14))
...
Dz_a1 <- VlnPlot(B_cells, c("DZ_a1"), pt.size  = 0.5, cols = turbo(14)) + ggtitle("DZ B cells") + NoLegend()
...
Lz_a1 <- VlnPlot(B_cells, c("LZ_a1"), pt.size  = 0.5, cols = turbo(14)) + ggtitle("LZ B cells") + NoLegend()
...
Lz_b<- VlnPlot(B_cells, c("LZ_b1"), pt.size  = 0.5, cols = turbo(14)) + ggtitle("LZ B cells") + NoLegend()
...
Int_a <- VlnPlot(B_cells, c("Int_a1"), pt.size  = 0.5, cols = turbo(14)) + ggtitle("Int_a") + NoLegend()
...
Int_c <- VlnPlot(B_cells, c("Int_c1"), pt.size  = 0.5, cols = turbo(14)) + ggtitle("Int_c") + NoLegend()
INT_c <- FeaturePlot(B_cells, c("Int_c1"), cols=col_con, reduction = "harmony.wnn.umap", pt.size  = 1.5) + ggtitle("Int_c")
FeaturePlot(B_cells, features = "Vim", reduction = "harmony.wnn.umap")
...
VlnPlot(B_cells, c("HALLMARK_INTERFERON_ALPHA_RESPONSE1"), pt.size  = 0.5, cols = turbo(14))
...
VlnPlot(B_cells, c("HALLMARK_INFLAMMATORY_RESPONSE1"), pt.size  = 0.5, cols = turbo(14))
...
DefaultAssay(B_cells) <- "RNA"
B_cells <-AddModuleScore(B_cells, features = HALLMARK_TNFA_SIGNALING_VIA_NFKB, name = "HALLMARK_TNFA_SIGNALING_VIA_NFKB", n = 22, search = TRUE)
Cluster_3 <-AddModuleScore(Cluster_3, features = HALLMARK_TNFA_SIGNALING_VIA_NFKB, name = "HALLMARK_TNFA_SIGNALING_VIA_NFKB", n = 22, search = TRUE)
VlnPlot(B_cells, c("HALLMARK_TNFA_SIGNALING_VIA_NFKB1"), pt.size  = 0.5, cols = turbo(14))
...
VlnPlot(B_cells, c("Steroidogenesis_mouse1"), pt.size  = 0.5, cols = turbo(14)) + NoLegend() + ggtitle("Enrichment for genes in the steroidogenesis pathway") + theme(plot.title = element_text(color="black", size=15, face="bold"))
