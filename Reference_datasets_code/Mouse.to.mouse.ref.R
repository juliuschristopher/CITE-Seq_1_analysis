## ##Query to mouse reference datasets####
library(SeuratDisk)
library(anndata)
...
setwd("~/Desktop/Reference_Datasets/Mouse_to_mouse")

####Query - Mouse (upper case gene names) to mouse (lower case gene name)####
experiment.lc <- LoadH5Seurat("experiment.lc.h5Seurat")
query <- experiment.lc
head(query[[]])

####Reference - mouse (lower case gene names)####
##1) Tabula
mu.ALL.1 <- LoadH5Seurat("./TabulaMuris/droplet.seurat.h5Seurat")
reference <- mu.ALL.1
head(reference[[]])
FeaturePlot(reference, features= "Cd19")
...
DefaultAssay(reference) <- "originalexp"
DefaultAssay(query) <- "RNA"
...
predictions <- TransferData(anchorset = anchors,refdata = Idents(reference))
